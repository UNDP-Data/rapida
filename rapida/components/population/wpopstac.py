...
def stac_search(stac_url=WPOP_STAC_URL,  collections:list[str] = None,
                year:int=None,  sex_category:str='total', age_group:str='total',project:str="Age and Sex Structures", resolution='100m'
                ):

    logger.debug(f'Searching data from {stac_url}  ')
    ...
    search_result = client.search(collections=collections, query=query)
    ...
    for itm in search_result.items():
        ...
        for asset_key, asset in itm.assets.items():
            ...
            itm_uris.append(asset.href)


        if itm_uris and not requested_variable_name in uris:
            uris[requested_variable_name] = {}

        uris[requested_variable_name][itm.collection_id] = itm_uris
